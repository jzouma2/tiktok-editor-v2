...
<div class="main">
  ...
  <div class="card">
    ...
    <div class="options-grid">
      ...
      <div class="opt-btn on" id="opt-vertical" onclick="toggle('vertical',this)"><span class="em">📱</span>TikTok format</div>
    </div>
  </div>
  <div class="card">
    <div class="label">Special instructions (optional)</div>
    <textarea id="instructions" rows="3" placeholder="e.g. make it fast-paced, start with the exterior shot, highlight the food close-ups..."></textarea>
  </div>
...
<script>
let files=[];let opts={silence:true,transitions:true,tips:true,vertical:true};
const emojis=['🎬','🍽️','🥂','🌆','✨','🎥','🍷','🌃'];
function handleFiles(input){files=[...files,...Array.from(input.files)];renderList();document.getElementById('goBtn').disabled=files.length===0;input.value='';}
function renderList(){
  document.getElementById('clipList').innerHTML=files.map((f,i)=>`<div class="clip-item"><div class="clip-icon">${emojis[i%emojis.length]}</div><div class="clip-info"><div class="clip-name">${f.name}</div><div class="clip-size">${(f.size/1024/1024).toFixed(1)} MB</div></div><button class="clip-remove" onclick="removeClip(${i})">✕</button></div>`).join('');
...
async function startEdit(){
  ...
  fd.append('clip_count',files.length);fd.append('options',JSON.stringify(opts));fd.append('instructions',document.getElementById('instructions').value);
  setProgress(5,`Uploading ${files.length} clip${files.length>1?'s':''}...`);
  try{const r=await fetch('/upload',{method:'POST',body:fd});const{job_id}=await r.json();setProgress(15,'Processing...');poll(job_id);}
  catch(e){showError('Upload failed. Please try again.');document.getElementById('goBtn').disabled=false;}
}
async function poll(id){
  const iv=setInterval(async()=>{
    try{const d=await(await fetch(`/status/${id}`)).json();setProgress(d.progress||15,d.message||'Processing...');
...
function showResult(id,tips){
  ...
  document.getElementById('resultVideo').src=url;document.getElementById('downloadBtn').href=url;
  document.getElementById('resultCard').classList.add('show');setProgress(100,'✅ Done! Your video is ready.');
  if(tips){document.getElementById('tipsText').textContent=tips;document.getElementById('tipsBox').style.display='block';}
  document.getElementById('goBtn').disabled=false;document.getElementById('resultCard').scrollIntoView({behavior:'smooth'});
}
...
def process(job_id, clip_paths, options, instructions):
    try:
        ...
        if options.get('silence'):
            upd(job_id, 40, 'Cutting silent moments...')
            out = os.path.join(UPLOAD_FOLDER, f"{job_id}_cut.mp4")
            working = cut_silences(working, out)

        if options.get('transitions'):
            upd(job_id, 60, 'Adding transitions...')
            out = os.path.join(UPLOAD_FOLDER, f"{job_id}_fade.mp4")
            working = add_fade(working, out)

        upd(job_id, 78, 'Exporting for TikTok...')
        vf = 'scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2:black' if options.get('vertical') else 'scale=trunc(iw/2)*2:trunc(ih/2)*2'
        run_ffmpeg(['-y', '-i', working, '-vf', vf, '-vcodec', 'libx264', '-acodec', 'aac', '-preset', 'fast', '-crf', '23', '-movflags', '+faststart', '-r', '30', output_path])

        tips = None
        if options.get('tips'):
            upd(job_id, 93, 'Generating viral tips...')
...
def merge_clips(clip_paths, output_path):
    try:
        ...
        for i, p in enumerate(clip_paths):
            ...
            run_ffmpeg(['-y', '-i', p, '-vf', 'scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2:black,fps=30', '-ar', '44100', '-ac', '2', '-vcodec', 'libx264', '-acodec', 'aac', '-preset', 'fast', '-crf', '23', n])
            normalized.append(n)
...
def cut_silences(input_path, output_path):
    try:
        ...
        run_ffmpeg(['-y', '-i', input_path, '-filter_complex', fs, '-map', '[outv]', '-map', '[outa]', output_path])
...
def add_fade(input_path, output_path):
    try:
        run_ffmpeg(['-y', '-i', input_path, '-vf', 'fade=t=in:st=0:d=0.3', '-c:a', 'copy', output_path])
        return output_path if os.path.exists(output_path) else input_path
    except:
        return input_path
...
def get_tips(instructions, clip_count):
    try:
        response = anthropic_client.messages.create(
            ...
            max_tokens=300,
            messages=[{"role": "user", "content": f"You are a viral TikTok expert for travel, food, fashion and lifestyle. Creator filmed {clip_count} clips. Instructions: '{instructions}'. Give 4 specific tips to make this go viral on TikTok. Focus on pacing, text overlays to add in TikTok app, trending sounds, and hooks. Under 150 words, be specific."}]
        )
